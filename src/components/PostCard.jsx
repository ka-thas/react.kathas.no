...
function PostCard({ slug, title, date, description }) {
  return (
    <Link
      to={`/blog/${slug}`}
      className="block px-5 py-4 rounded-xl bg-[rgba(150,200,150,0.1)] border border-black/10 no-underline text-inherit transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.2)] hover:border-accent-green/40"
    >
      <h2 className="inline-flex items-center gap-2 text-[1.1rem] font-bold m-0 p-0 border-none text-black leading-snug">
        {title}
        {isNew(date) && (
