...
function KaCaptcha() {
  ...
  return (
    <div className="w-[300px] border border-[rgba(255,255,255,0.15)] rounded-[6px] overflow-hidden bg-[rgba(20,38,28,0.92)] shadow-[0_4px_24px_rgba(0,40,0,0.4)] text-[0.9rem] select-none">
      <div className="flex flex-col gap-[2px] px-4 py-3 bg-accent-green/8 border-b border-[rgba(255,255,255,0.1)]">
      ...
      <div className="grid grid-cols-3 gap-[2px] bg-black/30 p-[2px]">
        {placeholders.map(({ id, src }) => {
          ...
          return (
            <button
              key={id}
              className={`relative aspect-square bg-[rgba(30,50,38,0.8)] border-none p-0 cursor-pointer overflow-hidden transition-[filter] duration-150 hover:[filter:brightness(1.15)]${isSelected ? " outline outline-3 outline-accent-green [outline-offset:-3px]" : ""}`}
              onClick={() => toggle(id)}
            ...
            >
              ...
              {isSelected && (
                <div className="absolute inset-0 flex items-center justify-center bg-accent-green/25">
                  <svg
                    className="w-7 h-7 text-accent-green drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)]"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="3"
      ...
      <div className="flex items-center justify-end gap-3 px-3 py-[0.6rem] border-t border-[rgba(255,255,255,0.08)]">
        {verified ? (
          <>
            <span className="flex items-center gap-[0.4rem] text-accent-green font-semibold text-[0.9rem]">
              <svg
                className="w-4 h-4"
                viewBox="0 0 24 24"
                fill="none"
          ...
          <button
            className="py-[0.35rem] px-4 rounded-[5px] bg-accent-green/18 text-accent-green font-semibold text-[0.9rem] cursor-pointer border border-accent-green/35 transition-colors duration-150 enabled:hover:bg-accent-green/28 disabled:opacity-35 disabled:cursor-default"
